...
```cmd
executor <ПутьКСкриптам>main.sbsl -Режим <РежимИсполнения> -ПутьДоРепозитория "<ОтносительныйПутьККорнюПроекта>" -Пауза <ОстановитьсяВКонце> -ПутьКНастройкам "<ПутьКФайламНастройки>" -ИменаИБ <ИменаИнформационныхБаз>
```
...
2. Создать файл инициализации и расположить его в каталоге bin.
   ...
   - Для **Windows** файл `init.bat` со следующим кодом
    ...
    ```cmd
    chcp 65001
    executor "..\scripts\1c_cicd\main.sbsl" -Режим init -ПутьДоРепозитория "..\\" -Пауза 1
    ```
